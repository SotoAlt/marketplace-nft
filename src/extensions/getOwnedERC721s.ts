...
export async function getOwnedERC721s(
  ...
  const { nextTokenIdToMint, startTokenId, totalSupply, ownerOf, getNFT } = await import(
  ...
  if (requestPerSec) {
    ...
    for (let i = 0; i < tokenIdsArrays.length; i++) {
      const data = await Promise.all(
        tokenIdsArrays[i].map((tokenId) => ownerOf({ contract, tokenId }).catch(() => ADDRESS_ZERO))
      );
    ...
    const owners = await Promise.all(
      allTokenIds.map((tokenId) => ownerOf({ contract, tokenId }).catch(() => ADDRESS_ZERO))
    );
